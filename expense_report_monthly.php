    ...
    <div>
        <div>
            <div class="mt-2 text-center">
                                ...
                                <hr>
                                <h4><b><?php echo $dt->format('F'); ?>, <?php echo $dt->format('Y'); ?> খর‌চের রি‌পোর্ট</b></h4>
            </div>
        </div>
    </div>
                        <?php
                        ...
                        if($admin_type == 0 OR $admin_type == 999) {
                            $query = "SELECT * FROM expenses,categories,sub_categories WHERE expenses.category_id = categories.category_id AND expenses.sub_category_id = sub_categories.sub_category_id AND MONTH(expenses.expense_date)='$month' GROUP BY expenses.category_id";
                        }else{
                            $query = "SELECT * FROM expenses,categories,sub_categories,admin_table WHERE expenses.category_id = categories.category_id AND expenses.sub_category_id = sub_categories.sub_category_id AND MONTH(expenses.expense_date)='$month' and admin_table.admin_type = categories.admin_type and admin_table.admin_type = '$admin_type' GROUP BY expenses.category_id";
                        }
