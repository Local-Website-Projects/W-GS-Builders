...
<div>
    <div>
        <div>
            <div class="mt-2 text-center">
                ...
                <hr>
                <h4><b><?php echo $dt->format('Y'); ?> স‌নের আয়ের রি‌পোর্ট</b></h4>
            </div>
        </div>
    </div>
    <?php
    ...
    if ($admin_type == 0 or $admin_type == 999) {
        $query = "SELECT * FROM incomes,categories,sub_categories WHERE incomes.cat_id = categories.category_id AND incomes.sub_cat_id = sub_categories.sub_category_id AND YEAR(incomes.date)='$year' GROUP BY incomes.cat_id";
    } else {
        $query = "SELECT * FROM incomes,categories,sub_categories,admin_table WHERE incomes.cat_id = categories.category_id AND incomes.sub_cat_id = sub_categories.sub_category_id AND YEAR(incomes.date)='$year' and admin_table.admin_type = categories.admin_type and admin_table.admin_type = '$admin_type' GROUP BY incomes.cat_id";
    }
        ...
        while ($row = mysqli_fetch_assoc($data)) {
            ...
            <div class="table-responsive p-2 text-center">
                ...
                <table class="table table-bordered text-center">
                    ...
                    <?php
                    $query1 = "SELECT SUM(incomes.amount) as sum_of_amount, sub_categories.sub_category_name,incomes.date FROM incomes,categories,sub_categories WHERE incomes.cat_id = categories.category_id AND incomes.sub_cat_id = sub_categories.sub_category_id AND YEAR(incomes.date)='$year' AND incomes.cat_id = '$categoryid' GROUP BY sub_categories.sub_category_id";
