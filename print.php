...
<div>
    ...
    if($admin_type == 0 OR $admin_type == 999){
        $query = "SELECT * FROM incomes,categories,sub_categories WHERE incomes.cat_id = categories.category_id AND incomes.sub_cat_id = sub_categories.sub_category_id AND DATE(incomes.date)='2021-07-10' GROUP BY incomes.cat_id";
    }else{
        $query = "SELECT * FROM incomes,categories,sub_categories,admin_table WHERE incomes.category_id = categories.category_id AND incomes.sub_category_id = sub_categories.sub_category_id AND DATE(incomes.date)='$date' and admin_table.admin_type = categories.admin_type and admin_table.admin_type = '$admin_type' GROUP BY incomes.category_id";
    }
