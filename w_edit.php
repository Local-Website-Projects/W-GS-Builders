...
<head>
    ...
    $w_id = $_GET['w_id'];
    $sql = "SELECT * FROM withdraw,categories,sub_categories WHERE withdraw.cat_id = categories.category_id AND withdraw.sub_cat_id = sub_categories.sub_category_id AND withdraw.w_id = '$w_id'";
    ...
    if (isset($_POST['update'])) {
        ...
        $update_sql = "UPDATE `withdraw` SET `cat_id`='$category_id',`sub_cat_id`='$sub_category_id',`amount`='$amount',`date`='$expense_date',`description`='$expense_note',`comment`='$comment' WHERE w_id = '$w_id' ";
                            ...
                            <div class="row">
                                <form method="post">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select js-select2" name="sub_category_id" id="subcategory" onchange="totalAmount(this.value);" required>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="amount" id ="remaining" class="form-control" value="<?php

                                        $subcat=$row['sub_category_id'];

                                        $q = "SELECT SUM(i.amount) as income FROM incomes as i WHERE i.sub_cat_id = $subcat";

                                        $result3 = mysqli_query($con, $q);

                                        while ($rows_result3 = mysqli_fetch_array($result3)) {
                                            $income = $rows_result3['income'];
                                        }

                                        $q1 = "SELECT SUM(w.amount) as withdraw FROM withdraw as w WHERE w.sub_cat_id = $subcat";
                                        $result4 = mysqli_query($con, $q1);
                                        while ($rows_result4 = mysqli_fetch_array($result4)){
                                            $withdraw = $rows_result4['withdraw'];
                                        }
